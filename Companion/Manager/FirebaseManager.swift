...
final class FirebaseManager {
    ...
    let storageReferenceToStudentsImages =
    Storage.storage().reference().child("students_images").child("\(UUID().uuidString).jpg")
    let storageReferenceToTeachersImages =
    Storage.storage().reference().child("teachers_images").child("\(UUID().uuidString).jpg")

    // MARK: - Init
    private init() { }

    // MARK: - API
    func fetchUserPartnerNameLabel(
    ...
    func fetchUserPartnerProfileImageView(
